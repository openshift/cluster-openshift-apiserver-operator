...
func CreateAndStoreTokenOfLife(t testing.TB, cs ClientSet) *oauthapiv1.OAuthAccessToken {
	t.Helper()
	{
	...
	tokenOfLife, err := cs.TokenClient.OAuthAccessTokens().Create(TokenOfLife(t))
	require.NoError(t, err)
	return tokenOfLife
}

func TokenOfLife(t testing.TB) *oauthapiv1.OAuthAccessToken {
	t.Helper()
...
func GetRawTokenOfLife(t testing.TB, clientSet encryption.ClientSet) string {
	t.Helper()
