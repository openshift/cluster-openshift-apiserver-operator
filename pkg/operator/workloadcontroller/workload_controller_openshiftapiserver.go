...
func syncOpenShiftAPIServer(c OpenShiftAPIServerOperator, originalOperatorConfig *operatorv1.OpenShiftAPIServer) (bool, error) {
	errors := []error{}
	...
	configMapModifiedObject, configMapModified, err := manageOpenShiftAPIServerConfigMap(c.kubeClient, c.kubeClient.CoreV1(), c.eventRecorder, operatorConfig)
	if err != nil {
	...
	imageImportCAModifiedObject, imageImportCAModified, err := manageOpenShiftAPIServerImageImportCA(c.openshiftConfigClient, c.kubeClient.CoreV1(), c.eventRecorder)
	if err != nil {
	...
	actualDaemonSet, _, err := manageOpenShiftAPIServerDaemonSet(c.kubeClient.AppsV1(), c.eventRecorder, c.targetImagePullSpec, operatorConfig, operatorConfig.Status.Generations, forceRollingUpdate)
	if err != nil {
	...
	currOperatorStatus, _, err := v1helpers.UpdateStatus(
		c.operatorClient,
	...
	if !v1helpers.IsOperatorConditionFalse(currOperatorStatus.Conditions, operatorv1.OperatorStatusTypeDegraded) {
		return true, nil
	}
	if !v1helpers.IsOperatorConditionFalse(currOperatorStatus.Conditions, operatorv1.OperatorStatusTypeProgressing) {
		return true, nil
	}
	if !v1helpers.IsOperatorConditionTrue(currOperatorStatus.Conditions, operatorv1.OperatorStatusTypeAvailable) {
		return true, nil
	}
...
// ca-bundle ConfigMap.
func manageOpenShiftAPIServerImageImportCA(openshiftConfigClient openshiftconfigclientv1.ConfigV1Interface, client coreclientv1.CoreV1Interface, recorder events.Recorder) (*corev1.ConfigMap, bool, error) {
...
func manageOpenShiftAPIServerConfigMap(kubeClient kubernetes.Interface, client coreclientv1.ConfigMapsGetter, recorder events.Recorder, operatorConfig *operatorv1.OpenShiftAPIServer) (*corev1.ConfigMap, bool, error) {
...
func manageOpenShiftAPIServerDaemonSet(client appsclientv1.DaemonSetsGetter, recorder events.Recorder, imagePullSpec string, operatorConfig *operatorv1.OpenShiftAPIServer, generationStatus []operatorv1.GenerationStatus, forceRollingUpdate bool) (*appsv1.DaemonSet, bool, error) {
